...
export function renderGamesTab(currentPlayer, myWallet, onWalletChange) {
  ...
  container.innerHTML = `
    ...
    <div class="games-grid">
      ...
      <button class="game-select-card" onclick="window.openGame('ppt')">
        ...
        <span class="game-select-desc">Ganá = +5 🪙</span>
      </button>
    </div>
  `;

  // ================================
  // PANTALLA COMPLETA TRAGAMONEDAS
  // ================================
    ...
    if (gameId === 'slots') {
      ...
      document.getElementById('btn-slot').onclick = async () => {
        ...
        const intervals = reels.map((reel, i) =>
          setInterval(() => { reel.textContent = weightedRandom(SLOT_SYMBOLS).emoji; reel.classList.add('spinning'); }, 80 + i*20)
        );
      ...
      content.innerHTML = `
        ...
        <div class="ppt-rules">
          <span class="ppt-rule win">✅ Ganás = <strong>+5 🪙</strong></span>
          <span class="ppt-rule tie">🤝 Empate = devuelve 🪙</span>
          <span class="ppt-rule lose">😹 Pusheen gana = <strong>−2 🪙</strong></span>
        </div>
        <div class="ppt-arena">
          <div class="ppt-side">
        ...
        document.getElementById(`ppt-${choice}`).onclick = async () => {
          ...
          setTimeout(async () => {
            ...
            if (result === 'win') {
              ...
              // Pusheen gana → te quita 2 monedas adicionales (ya pagaste 1 al inicio)
              await onWalletChange(-2);
              updateGFWallet();
              showGameMsg('ppt-result', `¡Pusheen ganó! ${pusheenChoice.emoji} vence a ${chosen.emoji} · −2 🪙 😹`, 'lose');
              pusheenEl.classList.add('ppt-winner'); playerEl.classList.add('ppt-loser');
            } else {
              // Empate → devuelve la moneda del costo
              await onWalletChange(1);
              updateGFWallet();
  ...
  // Actualizar wallet en pantalla de juego
